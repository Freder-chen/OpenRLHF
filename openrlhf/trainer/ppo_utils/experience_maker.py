...
class RemoteExperienceMaker:
        ...
        for experience, reward in zip(experiences, rewards):
            ...
            if self.advantage_estimator == "gae":
                ...
                if self.advantage_estimator == "reinforce" and args.gamma != 1.0:
                    decay_positions = experience.info.pop("decay_positions")
                    decay_powers = experience.info.pop("decay_powers")
                    ...
                    experience.returns = self.get_cumulative_returns(reward, args.gamma)
    ...
    def get_cumulative_returns(
        ...
        cumulative_return = torch.zeros(batch_size, device=rewards.device)
        for t in reversed(range(response_length)):
    ...
    def get_returns_with_decay(
        ...
        # Default: no decay -> all ones.
        discounts = torch.ones_like(rewards, dtype=rewards.dtype)
        if response_length > 0 and decay_positions is not None and decay_powers is not None:
            ...
            if valid.any():
                ...
                decay_exp.scatter_add_(1, decay_pos, decay_pow * valid.to(decay_pow.dtype))
